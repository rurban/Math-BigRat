...
sub bint {
    my ($self, $x, @r) = ref($_[0]) ? (ref($_[0]), $_[0]) : objectify(1, @_);
...
sub blsft {
    ...
    $xtmp -> blsft($y, $n);
    $xtmp = $self -> new($xtmp);                      # back to Math::BigRat
...
sub as_float {
    ...
    # objectify is costly, so avoid it
    ($self, $x, @r) = objectify(1, @_) unless ref $_[0];
